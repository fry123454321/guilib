...
// TODO: fix
const std::string DATA_PATH = "/home/builder/cpp/guilib/data/";
...
struct Incppect<SSL>::Impl {
    ...
    void run() {
        ...
        wsBehaviour.compression = uWS::CompressOptions::SHARED_COMPRESSOR;
        ...
        (*app).template ws<PerSocketData>("/incppect", std::move(wsBehaviour)
            ...
            res->writeHeader("Location", "/data/index.html");
            res->end();
        });
        ...
        // FileServer handler
        (*app).get("/data/*", [this](auto *res, auto *req) {
            const std::string url = std::string(req->getUrl());
            my_printf("url = '%s'\n", url.c_str());

            std::string subpath = std::string{req->getUrl().substr(6)};
            std::ifstream file(DATA_PATH + subpath);
            if (!file.good())
        ...
        for (auto & [clientId, cd] : clientData) {
            ...
            if (curBuffer.size() > 4) {
                if (curBuffer.size() == prevBuffer.size() && curBuffer.size() > 256) {
                    ...
                    if (socketData[clientId]->ws->send({ diffBuffer.data(), diffBuffer.size() }, uWS::OpCode::BINARY, doCompress) != WebSocket::SendStatus::SUCCESS) {
                    ...
                    if (socketData[clientId]->ws->send({ curBuffer.data(), curBuffer.size() }, uWS::OpCode::BINARY, doCompress) != WebSocket::SendStatus::SUCCESS) {
                        my_printf("[incpeect] warning: backpressure for client %d increased \n", clientId);
                    }
                }
